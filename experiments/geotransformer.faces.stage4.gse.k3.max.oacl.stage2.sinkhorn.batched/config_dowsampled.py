...
# set manually per run to avoid overwriting a previous experiment's output
_C.exp_name = osp.basename(_C.working_dir) + '.downsampled'
_C.output_dir = osp.join(_C.root_dir, 'output', _C.exp_name)
# repo-level output/ may not be writable outside the training container
if not os.access(osp.dirname(_C.output_dir), os.W_OK):
    _C.output_dir = osp.join(_C.working_dir, 'output', _C.exp_name)
...
_C.coarse_matching.entropy_weighting = False   # down-weight ambiguous superpoints by score-distribution entropy
_C.coarse_matching.entropy_temperature = 1.0   # scales exp(-entropy * temperature); higher = sharper suppression
...
_C.geotransformer.sigma_d = 0.16
...
_C.loss.weight_morph_loss = 100.0
_C.loss.weight_dense_loss = 100.0
