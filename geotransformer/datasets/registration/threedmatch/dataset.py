...
class ThreeDMatchPairDataset(torch.utils.data.Dataset):
    ...
    def __getitem__(self, index):
        ...
        morphed_full = self._load_point_cloud(metadata['pcd_morphed'])

        # get gt_z
        full_gt_z_path = osp.join(self.data_root, metadata['gt_z_path'])
        gt_z = torch.load(full_gt_z_path, weights_only=False)
